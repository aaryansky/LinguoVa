...
// Track selected language and level
...
document.getElementById('registerForm').addEventListener('submit', async (e) => {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/auth/register`, {
      ...
      body: JSON.stringify({ name, email, password, language: selectedLang, level: selectedLevel })
    });
    ...
    errorEl.textContent = 'Server error. Make sure backend is running.';
...
document.getElementById('btnGoogle')?.addEventListener('click', (e) => {
  e.preventDefault();
  showGoogleDirectModal(true); // true = relative to register.html (inside pages/)
});
  ...
  formEl.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.role === 'admin') {
          window.location.href = isInsidePagesDir ? 'admin.html' : 'admin.html'; // since we are already in pages/
        } else {
          window.location.href = isInsidePagesDir ? 'dashboard.html' : 'dashboard.html';
        }
      } else {
